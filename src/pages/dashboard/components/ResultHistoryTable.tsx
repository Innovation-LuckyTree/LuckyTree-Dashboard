...
export const ResultHistoryTable: FC =() => {
  ...
  const [filteredData, setFilteredData] = useState<ResultHistory[]>(mockResultHistoryData);
  ...
  const columns: TableColumnsType<ResultHistory> = [
    {
      title: 'Game Type',
      width:120,
      dataIndex: 'gameType',
    ...
    },
    {
      ...
      ...getColumnSearchProps('processedBy'),
    },
    {
      ...
      ...getColumnSearchProps('dateProcessed'),
      render: (text) => new Date(text).toLocaleDateString(),
    },
  ];

  const onChange: TableProps<ResultHistory>['onChange'] = (pagination, filters, sorter, extra) => {
